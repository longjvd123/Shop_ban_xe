...
import com.dinhlong.vipprocosmetics.entity.Item;
...
public class CartServlet extends HttpServlet {
	...
	protected void doGet_Buy(HttpServletRequest request, HttpServletResponse response)
		...
		if (session.getAttribute("cart") == null) {
			...
			cart.add(new Item(cosmeticsDao.findById(request.getParameter("id")), 1));
			session.setAttribute("cart", cart);
		} else {
			List<Item> cart = (List<Item>) session.getAttribute("cart");
			int index = isExisting(request.getParameter("id"), cart);
	...
	private int isExisting(String id, List<Item> cart) {
		for (int i = 0; i < cart.size(); i++) {
			if (cart.get(i).getCosmetics().getId().equalsIgnoreCase(id)) {
				return i;
			}
		}
		return -1;
